<?php
/**
 * Plugin Name: ACF Module Inspector
...
if ( ! class_exists( 'ACF_Module_Inspector' ) ) :
	...
	class ACF_Module_Inspector {
		...
		function init_scripts() {
			wp_enqueue_script( 'acf-module-inspector-admin', plugins_url( 'js/acf-module-inspector-admin.js', __FILE__ ), array( 'jquery' ) );
			wp_localize_script(
                'acf-module-inspector-admin',
                'ACFModuleInspector',
                [
			...
			wp_enqueue_style( 'acf-module-inspector-admin', plugins_url( 'css/acf-module-inspector.css', __FILE__ ), array(), filemtime( plugin_dir_path( __FILE__ ) . '/css/acf-module-inspector.css' ) );
		}
		...
		function include_tools() {
			include_once $path . 'acf-module-inspector-admin.php';
		}
...
if ( ! function_exists( 'acf_module_inspector_is_acf_installed' ) ) {
	function acf_module_inspector_is_acf_installed() {
		...
		if ( is_admin() && ( ! class_exists( 'acf_pro' ) && current_user_can( 'activate_plugins' ) ) && is_plugin_active( $file ) ) {
			add_action( 'admin_notices', function() { echo '<div class="error"><p>' . sprintf( __( 'Activation failed: <strong>Advanced Custom Fields Pro</strong> must be activated to use the <strong>ACF Module Inspector</strong>. %sVisit your plugins page to install and activate.', 'acf-module-inspector' ), '<a href="' . admin_url( 'plugins.php#debug-bar' ) . '">' ) . '</a></p></div>'; } );
